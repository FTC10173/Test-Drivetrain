...
@Config
public final class PoseEstimator implements Localizer {
    public static class Params {
        ...
        public double maxVisionStaleness = 20;
        public double visionWeight = 0.75;
        public double maxVisionJump = 18;
    }
    ...
    public PoseEstimator(HardwareMap hardwareMap, double inPerTick, Pose2d initialPose) {
        ...
        FlightRecorder.write("POSE_ESTIMATOR_THREE_DEAD_WHEEL_PARAMS", PARAMS);
    ...
    public void addLimelight(LLResult result) {
        ...
        if (result.getStaleness() > PARAMS.maxVisionStaleness) return;
        ...
        Translation2d currentTrans = new Translation2d(
                pose.position.x, pose.position.y
        );

        Translation2d llTrans = new Translation2d(
                llX, llY
        );


        double error = currentTrans.getDistance(llTrans);

        // Reject huge jumps
        if (error > PARAMS.maxVisionJump) return;

        double alpha = PARAMS.visionWeight;

        pose = new Pose2d(
                (pose.position.x * (1 - alpha)) + (llX * alpha),
                (pose.position.y * (1 - alpha)) + (llY * alpha),
